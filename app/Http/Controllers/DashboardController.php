...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $listKGB = $trackers->where('kategori', 'KGB')->sortBy(function ($item) {
            ...
            if ($item->status_saat_ini === 'Upload E-HRM') {
                return 2;
            }
        ...
        $kpStruktural = $listKenaikanPangkat->filter(function ($item) {
            if ($item->kategori === 'KP_Struktural') {
                return true;
            }
            if (in_array($item->kategori, ['KP_Jafung', 'KP_Reguler'])) {
                return false;
            }
        ...
        $kpFungsional = $listKenaikanPangkat->filter(function ($item) {
            if ($item->kategori === 'KP_Jafung') {
                return true;
            }
            if (in_array($item->kategori, ['KP_Struktural', 'KP_Reguler'])) {
                return false;
            }
        ...
        $kpReguler = $listKenaikanPangkat->filter(function ($item) {
            if ($item->kategori === 'KP_Reguler') {
                return true;
            }
            if (in_array($item->kategori, ['KP_Struktural', 'KP_Jafung'])) {
                return false;
            }
